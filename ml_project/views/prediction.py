        ...
        go.Indicator(
            ...
            gauge={
                "axis": {
                    ...
                    "ticktext": CLASSES_DPE_MAPPING,
                },
                "bar": {"color": COLORS_DPE[safe_index]},
                "steps": [
                    {"range": [i + 1, i + 2], "color": COLORS_DPE[i]}
                    for i in range(len(CLASSES_DPE_MAPPING))
                ],
            },
            title={
                "text": f"Classe DPE : {classe_label}",
                "font": {"color": COLORS_DPE[safe_index], "size": 20},
            },
        )
    )
    fig.update_layout(
        template="plotly_dark", height=300, margin=dict(l=20, r=20, t=60, b=20)
    )
...
def create_conso_gauge(conso_pred):
    ...
    fig = go.Figure(
        go.Indicator(
            ...
            gauge={
                "axis": {
                    "range": [0, max_conso_chart],
                    "tickvals": [0, 5000, 15000, max_conso_chart],
                },
                "bar": {"color": "#3498db"},
                "steps": [
                    ...
                    {
                        "range": [15000, max_conso_chart],
                        "color": "rgba(231, 76, 60, 0.2)",
                    },
                ],
            },
            title={
                "text": "Consommation Estimée (kWh/an)",
                "font": {"color": "#3498db", "size": 20},
            },
        )
    )
    fig.update_layout(
...
def show_page():
    ...
    # --- FORMULAIRE UNIQUE ---
    st.markdown(
        "<h3 style='color:#f1c40f; text-align:center;'>Caractéristiques du logement</h3>",
        unsafe_allow_html=True,
    )
    ...
    with col1:
        surface_habitable_logement = st.number_input(
            "Surface habitable (m²) :", min_value=10, max_value=500, value=80, step=5
        )
        ...
        nombre_appartement_cat = st.selectbox(
            "Type de bâtiment :",
            [
    ...
    with col2:
        type_energie_n1 = st.selectbox(
            "Type d'énergie principale (l'année dernière) :",
            [
                "Gaz naturel",
                "Électricité",
                ...
                "Bois et biomasse",
                "Fioul",
                "Gaz (GPL/Propane/Butane)",
                "Charbon",
            ],
        )
        type_energie_principale_chauffage = st.selectbox(
            "Type d'énergie principale (cette année) :",
            [
                "Gaz naturel",
                "Électricité",
                "Réseau de chauffage urbain",
    ...
    predict_button = st.button(
        "🚀 Lancer la prédiction complète", type="primary", use_container_width=True
    )
    ...
    if predict_button:
            ...
            with st.spinner("🔮 Étape 1/2 : Calcul de la classe DPE..."):
                ...
                if response_dpe.status_code == 200:
                    ...
                    if dpe_prediction is not None:
                        ...
                        st.error(
                            "❌ Erreur : Clé 'prediction_DPE_index' manquante dans la réponse"
                        )
                        return
                else:
                    st.error(
                        f"❌ Erreur API DPE ({response_dpe.status_code}): {response_dpe.text}"
                    )
                    return
        ...
        try:
            with st.spinner(
                "💡 Étape 2/2 : Estimation de la consommation énergétique..."
            ):
                response_conso = requests.post(
                    API_URL_CONSO, json=data_for_conso, timeout=30
                )
                ...
                if response_conso.status_code == 200:
                    ...
                    if conso_pred is not None:
                        ...
                        st.error(
                            "❌ Erreur : Clé 'conso_predite_kwh' manquante dans la réponse"
                        )
                        return
                else:
                    st.error(
                        f"❌ Erreur API Consommation ({response_conso.status_code}): {response_conso.text}"
                    )
                    return

        except requests.exceptions.ConnectionError:
            st.error(
                "❌ Impossible de se connecter à l'API Consommation sur le port 5000"
            )
            return
        except requests.exceptions.Timeout:
        ...
        if dpe_prediction is not None and conso_pred is not None:
            ...
            with results_container:
                st.markdown("---")
                st.markdown(
                    "<h2 style='text-align:center; color:#2ecc71;'>📊 Résultats de la Prédiction</h2>",
                    unsafe_allow_html=True,
                )
